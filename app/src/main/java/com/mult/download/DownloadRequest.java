...
public class DownloadRequest implements Comparable<DownloadRequest> {
    ...
    private DownloadStatusListener mDownloadStatusListenerV1;
    ...
    @Deprecated
    private DownloadRequest(Uri uri) {
        if (uri == null) {
    ...
    public String getLocalPath() {
        if (mLocalPath == null)
            mLocalPath = mContext.getCacheDir().getPath();
        return mLocalPath;
    }

    public DownloadRequest setLocalPath(String filePath) {
        if (filePath != null) {
            mLocalPath = filePath;
        } else {
            mLocalPath = mContext.getCacheDir().getPath();
